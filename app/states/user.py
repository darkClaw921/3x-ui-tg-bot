...
class BuyFlow(StatesGroup):
    ...
    ``confirming`` (user reviews and either pays or applies a promo) →
    ``entering_promo`` (text input of a promo code) → back to
    ...
    whose payload carries ``plan_id`` / ``promo_id``.
    """
...
class PromoActivate(StatesGroup):
    ...
    Flow: ``waiting_code`` (text input) → state cleared once a
    subscription is provisioned (or the user cancels).
    """
